...
%%  Hyperlinks.
%%  Link text is black, but we've customized the underline color using a custom command, `\link`.
\usepackage{hyperref}
\hypersetup{
  colorlinks=true,
  urlcolor=black,
...
\begin{document}
  ...
  \begin{center}
    {\scshape\MakeUppercase{\textls[-50]{\Huge Kieran Potts}}}
  \end{center}
  ...
  %    {2017}
  %  \resumeListStart
  %    \resumeItem{Rewrote all user-facing documentation to improve accuracy and readability}
  %    \resumeItem{Reverse engineered CurrencyCloud's public web service to create a complete OpenAPI specification}
  %  \resumeListEnd
  %\end{minipage}
